...
from output_builders.visa_questions import (
    ProcessQuestion,
    BaseQuestions,
    MissingQuestions,
    ConfuseQuestions,
...
class WhatsappFlow:
    ...
    def generate_analyze(self , lead_id , current_field , content):
        ...
        if current_field == "name":
            ...
            if isinstance(content , str):
                ...
                self.messages.add_lead_message(lead_id=lead_id , role="user" , content=content["id"].replace("eligibility_" , ""))
                return {"status" : "found" , "value" : content["id"].replace("eligibility_" , "")}
    ...
    def advance_on_found(self , ai_response , lead_info , content):
            ...
            if ai_response["status"] == "found":
                if lead_info["current_field"] == "pre_flow":
                    ...
                    lead_info["current_field"] = content["id"].replace("goal_" , "")
    ...
    def handle_unresolved_fallbacks(self , ai_response , lead_info):
        ...
        if ai_response["status"] == "missing" or ai_response["status"] == "confused":
            if lead_info["question_reason"] == "regular_fallback":
                ...
                if lead_info["current_field"] == "urgency":
                    lead_info["current_field"] = "booking_flow"
    ...
    def apply_message_score(self , lead_info , current_field , ai_analyze_response , reason , question_index):
        if current_field == "name":
            return
        ...
        lead_message_score = self.message_scorer.score_visa_message(message_to_rank=ai_analyze_response , field=current_field , reason=reason , question_index=question_index)
