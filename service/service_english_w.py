...
from data_access.postgres.appointment_context_dpdate_repository import AppointmentsContextRepository
        ...
        try:
            ...
            if isinstance(booking_result , dict):
                ...
                if question["status"] == "booking":
                    self.db.commit()
                    booking_r = self.booking_flow.process_booking_flow(lead_id=prepare_lead_context['lead_base_data']['lead_id'] , content=content)
                    print("BOOKING TRANSITION RESULT:",booking_r , flush=True)
                    return booking_r
                self.db.commit()
                return question
